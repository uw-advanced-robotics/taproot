...
void WiggleDriveCommand::initialize()
{
    ...
    turretCurAngle = aruwlib::algorithms::limitVal(turretCurAngle,
            -WIGGLE_MAX_ROTATE_ANGLE, WIGGLE_MAX_ROTATE_ANGLE);
    ...
    // are still in phase.
    startTimeForAngleOffset = asinf(turretCurAngle / WIGGLE_MAX_ROTATE_ANGLE)
            * WIGGLE_PERIOD / (2.0f * aruwlib::algorithms::PI);
...
{
    return WIGGLE_MAX_ROTATE_ANGLE
            * sinf((2.0f * aruwlib::algorithms::PI / WIGGLE_PERIOD) * t);
}
...
void WiggleDriveCommand::execute()
{
    ...
    if (turret->isTurretOnline())
    {
        ...
        {
            desiredAngleError = aruwlib::algorithms::limitVal(turretYawAngle,
...
void WiggleDriveCommand::end(bool)  // NOLINT
{
