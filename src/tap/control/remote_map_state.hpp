...
public:
    ...
    /**
     * Generic constructor that takes all possible remote map state configurations for maximum configurability, initializes
     * the RemoteMapState with all given information.
     * 
     * @param[in] leftss The switch state for the left switch. If `UNKNOWN`, the switch state will not be initialized.
     * @param[in] rightss The switch state for the right switch. If `UNKNOWN`, the switch state will not be initialized.
     ...
     * @param[in] mouseButtonLeftPressed Initialize the RemoteMapState to match a remote map state when the left mouse button is pressed.
     * @param[in] mouseButtonRightPressed Initialize the RemoteMapState to match a remote map state when the right mouse button is pressed.
     * @note `keySet` and `negKeySet` must be mutally exclusive sets, otherwise the `negKeySet` will not be properly initialized.
     */
    RemoteMapState(
