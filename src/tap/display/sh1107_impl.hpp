...
modm::ResumableResult<bool> tap::display::Sh1107<SPI, A0, Reset, Width, Height, Flipped, RotateDegrees>::
    updateNonblocking()
{
    RF_BEGIN(0);
    ...
    for (y = 0; y < (Height / 8); ++y)
    {
        ...
        for (x = 0; x < Width; ++x)
        {
            if (RotateDegrees == 90 || RotateDegrees == 270) {
                RF_CALL(spi.transfer(this->buffer[x][y]));
            } else {
...
template <
    ...
    bool Flipped,
    unsigned int RotateDegrees>
void tap::display::Sh1107<SPI, A0, Reset, Width, Height, Flipped, RotateDegrees>::update()
{
...
template <
    ...
    bool Flipped,
    unsigned int RotateDegrees>
void tap::display::Sh1107<SPI, A0, Reset, Width, Height, Flipped, RotateDegrees>::setInvert(bool invert)
{
    a0.reset();
...
template <
    ...
    bool Flipped,
    unsigned int RotateDegrees>
void tap::display::Sh1107<SPI, A0, Reset, Width, Height, Flipped, RotateDegrees>::initializeBlocking()
{
    a0.setOutput();
    reset.setOutput();
