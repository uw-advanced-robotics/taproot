...
void fInitializeMagCalibration(struct MagCalibration *pthisMagCal, struct MagBuffer *pthisMagBuffer)
{
    ...
    // initialize the array of (MAGBUFFSIZEX - 1) elements of 100 * tangents used for buffer
    // indexing entries cover the range 100 * tan(-PI/2 + PI/MAGBUFFSIZEX), 100 * tan(-PI/2 +
    // 2*PI/MAGBUFFSIZEX) to 100 * tan(-PI/2 + (MAGBUFFSIZEX - 1) * PI/MAGBUFFSIZEX). for
    // MAGBUFFSIZEX=12, the entries range in value from -373 to +373
    for (i = 0; i < (MAGBUFFSIZEX - 1); i++)
        pthisMagBuffer->tanarray[i] =
            (int16_t)(100.0F * tanf(PI * (-0.5F + (float)(i + 1) / MAGBUFFSIZEX)));

    // check to see if the stored magnetic calibration has been erased
    // the standard value for erased flash is 0xFF in each byte but for portability check against
    // 0x12345678 flash has been erased and no magnetic calibration is present initialize the
    // magnetic calibration in RAM to null default
...
// the uncalibrated measurements iBs are stored in the buffer but the calibrated measurements iBc
    ...
    int32 loopcounter)
{
    ...
    // map tangent ratios to bins j and k using equal angle bins: C guarantees left to right
    // execution of the test and add an offset of MAGBUFFSIZEX bins to k to mimic atan2 on this
    // ratio j will vary from 0 to MAGBUFFSIZEX - 1 and k from 0 to 2 * MAGBUFFSIZEX - 1
    j = k = 0;
    ...
    // case 1: buffer is full and this bin has a measurement: over-write without increasing number
    // of measurements this is the most common option at run time
    if ((pthisMagBuffer->iMagBufferCount == MAXMEASUREMENTS) && (pthisMagBuffer->index[j][k] != -1))
    {
        // store the fast (unaveraged at typically 200Hz) integer magnetometer reading into the
        // buffer bin j, k
        for (i = CHX; i <= CHZ; i++)
        {
    ...
    // case 2: the buffer is full and this bin does not have a measurement: store and retire the
    // oldest this is the second most common option at run time
    if ((pthisMagBuffer->iMagBufferCount == MAXMEASUREMENTS) && (pthisMagBuffer->index[j][k] == -1))
    {
        // store the fast (unaveraged at typically 200Hz) integer magnetometer reading into the
        // buffer bin j, k
        ...
        for (j = 0; j < MAGBUFFSIZEX; j++)
        {
            ...
            {
                // check if the time stamp is older than the oldest found so far (normally fails
                // this test)
    ...
    // case 4: buffer is not full and this bin has a measurement: over-write if close or try to slot
    // in elsewhere if not close to the other measurements so as to create a mesh at power up
    if ((pthisMagBuffer->iMagBufferCount < MAXMEASUREMENTS) && (pthisMagBuffer->index[j][k] != -1))
    {
        ...
        {
            // reset the flag denoting that the current measurement is close to any measurement in
            // the buffer
            itooclose = 0;
            ...
            while (!itooclose && (j < MAGBUFFSIZEX))
            {
                ...
                while (!itooclose && (k < MAGBUFFSIZEY))
                {
                    ...
                    if (pthisMagBuffer->index[j][k] != -1)
                    {
                        ...
                        {
                            idelta +=
                                abs((int32)pthisMag->iBs[i] - (int32)pthisMagBuffer->iBs[i][j][k]);
                        }
...
// function maps the uncalibrated magnetometer data fBs (uT) onto calibrated averaged data fBc (uT),
// iBc (counts)
void fInvertMagCal(struct MagSensor *pthisMag, struct MagCalibration *pthisMagCal)
{
    ...
    for (i = CHX; i <= CHZ; i++)
    {
        pthisMag->fBc[i] = pthisMagCal->finvW[i][CHX] * ftmp[CHX] +
                           pthisMagCal->finvW[i][CHY] * ftmp[CHY] +
                           pthisMagCal->finvW[i][CHZ] * ftmp[CHZ];
    ...
    int32 loopcounter)
{
    ...
    if (!pthisMagCal->iCalInProgress)
    {
        ...
        // try one calibration attempt with the best model available given the number of
        // measurements
            ...
            (!pthisMagCal->i10ElementSolverTried))
        {
            ...
            (pthisMagBuffer->iMagBufferCount >= MINMEASUREMENTS7CAL) &&
            (!pthisMagCal->i7ElementSolverTried))
        {
            ...
            (pthisMagBuffer->iMagBufferCount >= MINMEASUREMENTS4CAL) &&
        ...
        else if (!pthisMagCal->iInitiateMagCal && !(loopcounter % (CAL_INTERVAL_SECS * FUSION_HZ)))
        {
            if (pthisMagBuffer->iMagBufferCount >= MINMEASUREMENTS10CAL)
            {
                pthisMagCal->i10ElementSolverTried = true;
    ...
    {
        // the geomagnetic field strength must be in range (earth is 22uT to 67uT) with reasonable
        // fit error
        if ((pthisMagCal->ftrB >= MINBFITUT) && (pthisMagCal->ftrB <= MAXBFITUT) &&
            (pthisMagCal->ftrFitErrorpc <= 15.0F))
        {
            // the fit error must be improved or be from a more sophisticated solver but still 5
            // bars (under 3.5% fit error)
            if ((pthisMagCal->ftrFitErrorpc <= pthisMagCal->fFitErrorpc) ||
                ((pthisMagCal->iNewCalibrationAvailable > pthisMagCal->iValidMagCal) &&
                 (pthisMagCal->ftrFitErrorpc <= 3.5F)))
            {
        ...
        {
            // the magnetic buffer is presumed corrupted so clear out all measurements and restart
    ...
    // age the existing fit error very slowly to avoid one good calibration locking out future
    // updates. this prevents a calibration remaining for ever if a unit is never powered down
...
void fUpdateMagCalibration4Slice(
    struct MagCalibration *pthisMagCal,
    struct MagBuffer *pthisMagBuffer,
    struct MagSensor *pthisMag)
{
    ...
    if (pthisMagCal->itimeslice == 0)
    {
        ...
        // zero on and above diagonal matrix X^T.X (in fmatA), vector X^T.Y (in fvecA), scalar Y^T.Y
        // (in fYTY)
        ...
        for (i = 0; i < 3; i++)
        {
            if (pthisMagCal->iSumBs[i] >= 0)
                pthisMagCal->iMeanBs[i] = (pthisMagCal->iSumBs[i] + ((int32)iM >> 1)) / (int32)iM;
            else
                pthisMagCal->iMeanBs[i] = (pthisMagCal->iSumBs[i] - ((int32)iM >> 1)) / (int32)iM;
        }
    ...
    else if ((pthisMagCal->itimeslice >= 1) && (pthisMagCal->itimeslice <= MAGBUFFSIZEX))
    {
        ...
        // accumulate the measurement matrix elements XTX (in fmatA), XTY (in fvecA) and YTY on the
        // zero mean measurements
        ...
        for (j = 0; j < MAGBUFFSIZEY; j++)
        {
            if (pthisMagBuffer->index[i][j] != -1)
            {
                ...
                for (k = 0; k < 3; k++)
                    iBsZeroMean[k] =
                        (int32)pthisMagBuffer->iBs[k][i][j] - (int32)pthisMagCal->iMeanBs[k];
                ...
                fBsZeroMeanSq = (float)(iBsZeroMean[CHX] * iBsZeroMean[CHX] +
                                        iBsZeroMean[CHY] * iBsZeroMean[CHY] +
                                        iBsZeroMean[CHZ] * iBsZeroMean[CHZ]);
                for (k = 0; k < 3; k++)
    ...
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX + 1))
    {
        ...
        {
            for (j = 0; j <= i; j++)
                pthisMagCal->fmatB[i][j] = pthisMagCal->fmatB[j][i] = pthisMagCal->fmatA[i][j] =
                    pthisMagCal->fmatA[j][i];
        }
    ...
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX + 2))
    {
        ...
        // the trial inverse soft iron matrix invW always equals the identity matrix for 4 element
        // calibration
        ...
        for (i = CHX; i <= CHZ; i++) pthisMagCal->ftrV[i] = ftmp * pthisMagCal->fvecB[i];

        // compute the geomagnetic field strength B (uT)
        pthisMagCal->ftrB = pthisMagCal->fvecB[3] * pthisMag->fuTPerCount * pthisMag->fuTPerCount;
        for (i = CHX; i <= CHZ; i++)
        ...
        for (i = CHX; i <= CHZ; i++) pthisMagCal->ftrV[i] += (float)pthisMagCal->iSumBs[i] * ftmp;
        ...
        for (i = 1; i < 4; i++) fE += pthisMagCal->fvecB[i] * pthisMagCal->fvecA[i];
        ...
        for (i = 0; i < 4; i++) fE += pthisMagCal->fvecB[i] * pthisMagCal->fvecA[i];

        // normalize fit error to the number of measurements and take square root to get error in
        // uT^2
        fE = sqrtf(fabs(fE) / (float)pthisMagBuffer->iMagBufferCount) * pthisMag->fuTPerCount *
             pthisMag->fuTPerCount;

        // obtain dimensionless error by dividing square square of the geomagnetic field and convert
        // to percent
...
void fUpdateMagCalibration7Slice(
    struct MagCalibration *pthisMagCal,
    struct MagBuffer *pthisMagBuffer,
    struct MagSensor *pthisMag)
{
...
#define MATRIX_7_SIZE 7
    ...
    // time slice 0: 18.1K KL25Z ticks for 300 measurements = 0.38ms on KL25Z (variable) stored in
    // systick[0] zero measurement matrix and calculate the mean values in the magnetic buffer
    if (pthisMagCal->itimeslice == 0)
    {
        ...
        for (i = 0; i < MATRIX_7_SIZE; i++)
            for (j = i; j < MATRIX_7_SIZE; j++) pthisMagCal->fmatA[i][j] = 0.0F;
    ...
    // time slices 1 to MAGBUFFSIZEX * MAGBUFFSIZEY: accumulate matrices: 8.6K KL25Z ticks = 0.18ms
    // (with max stored in systick[1])
    else if (
        (pthisMagCal->itimeslice >= 1) && (pthisMagCal->itimeslice <= MAGBUFFSIZEX * MAGBUFFSIZEY))
    {
        // accumulate the symmetric matrix fmatA on the zero mean measurements
        i = (pthisMagCal->itimeslice - 1) /
            MAGBUFFSIZEY;  // matrix row i ranges 0 to MAGBUFFSIZEX-1
        j = (pthisMagCal->itimeslice - 1) %
            MAGBUFFSIZEY;  // matrix column j ranges 0 to MAGBUFFSIZEY-1
        if (pthisMagBuffer->index[i][j] != -1)
        {
            // set fvecA to be vector of zero mean measurements and their squares
            for (k = 0; k < 3; k++)
            {
                pthisMagCal->fvecA[k + 3] =
                    (float)((int32)pthisMagBuffer->iBs[k][i][j] - (int32)pthisMagCal->iMeanBs[k]);
                pthisMagCal->fvecA[k] = pthisMagCal->fvecA[k + 3] * pthisMagCal->fvecA[k + 3];
            }

            // update non-zero elements fmatA[0-2][6] of fmatA ignoring fmatA[6][6] which is set
            // later. elements fmatA[3-5][6] are zero as a result of subtracting the mean value
            for (k = 0; k < 3; k++) pthisMagCal->fmatA[k][6] += pthisMagCal->fvecA[k];
    ...
    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 1: 0.8K ticks on KL25Z = 0.02ms on KL25Z (constant)
    // (stored in systick[2]) re-enable magnetic buffer for writing and prepare fmatA, fmatB, fvecA
    // for eigendecomposition
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 1))
    {
        // set fmatA[6][6] to the number of magnetic measurements found
        pthisMagCal->fmatA[MATRIX_7_SIZE - 1][MATRIX_7_SIZE - 1] =
        ...
        for (i = 1; i < MATRIX_7_SIZE; i++)
            for (j = 0; j < i; j++) pthisMagCal->fmatA[i][j] = pthisMagCal->fmatA[j][i];

        // set matrix of eigenvectors fmatB to identity matrix and eigenvalues vector fvecA to
        // diagonal elements of fmatA
        for (i = 0; i < MATRIX_7_SIZE; i++)
        {
            for (j = 0; j < MATRIX_7_SIZE; j++) pthisMagCal->fmatB[i][j] = 0.0F;
    ...
    // repeating 21 time slices MAGBUFFSIZEX * MAGBUFFSIZEY + 2 to MAGBUFFSIZEX * MAGBUFFSIZEY + 22
    // inclusive to perform the eigendecomposition of the measurement matrix fmatA. 19.6k ticks =
    // 0.41ms on KL25Z (with max stored in systick[3]). for a 7x7 matrix there are 21 above diagonal
    // elements: 6+5+4+3+2+1+0=21
    else if (
        (pthisMagCal->itimeslice >= (MAGBUFFSIZEX * MAGBUFFSIZEY + 2)) &&
        (pthisMagCal->itimeslice <= (MAGBUFFSIZEX * MAGBUFFSIZEY + 22)))
    {
        // set k to the matrix element in range 0 to 20 to be zeroed and used it to set row i and
        // column j
        ...
        // increment the time slice for the next iteration
        (pthisMagCal->itimeslice)++;
    }  // end of time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 2 to MAGBUFFSIZEX * MAGBUFFSIZEY + 22
       // inclusive

    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 23: 2.6k ticks on KL25Z = 0.05ms on KL25Z (constant)
    // (stored in systick[4]) compute the sum of the absolute values of above diagonal elements in
    // fmatA as eigen-decomposition exit criterion
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 23))
    {
        // sum residue of all above-diagonal elements
        fresidue = 0.0F;
        for (i = 0; i < MATRIX_7_SIZE; i++)
            for (j = i + 1; j < MATRIX_7_SIZE; j++) fresidue += fabsf(pthisMagCal->fmatA[i][j]);

        // determine whether to re-enter the eigen-decomposition or skip to calculation of the
        // calibration coefficients
        if (fresidue > 0.0F)
            // continue the eigen-decomposition
            (pthisMagCal->itimeslice) = MAGBUFFSIZEX * MAGBUFFSIZEY + 2;
        else
            // continue to compute the calibration coefficients since the eigen-decomposition is
            // complete
            (pthisMagCal->itimeslice)++;
    }  // end of time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 23

    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 24: 27.8k ticks = 0.58ms on KL25Z (constant) (stored
    // in systick[5]) compute the calibration coefficients from the solution eigenvector
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 24))
    {
        ...
        // the ellipsoid fA must have positive determinant but the eigensolver can equally easily
        // return a negated normalized eigenvector without changing the eigenvalue. compute the
        // determinant of ellipsoid matrix A from first three elements of eigenvector and negate the
        // eigenvector if negative.
        fdetA = pthisMagCal->fmatB[CHX][imin] * pthisMagCal->fmatB[CHY][imin] *
        ...
        // set diagonal elements of ellipsoid matrix A to the solution vector imin with smallest
        // eigenvalue and zero off-diagonal elements since these are always zero in the 7 element
        // model compute the hard iron offset fV for zero mean data (counts)
        for (i = CHX; i <= CHZ; i++)
            pthisMagCal->ftrV[i] =
                -0.5F * pthisMagCal->fmatB[i + 3][imin] / pthisMagCal->fmatB[i][imin];

        // set ftmp to the square of the geomagnetic field strength fB (counts) corresponding to
        // current value of fdetA
        ftmp = -pthisMagCal->fmatB[6][imin];
        for (i = CHX; i <= CHZ; i++)
            ftmp += pthisMagCal->fmatB[i][imin] * pthisMagCal->ftrV[i] * pthisMagCal->ftrV[i];

        // calculate the trial normalized fit error as a percentage normalized by the geomagnetic
        // field strength squared
        pthisMagCal->ftrFitErrorpc =
            50.0F *
            sqrtf(fabsf(pthisMagCal->fvecA[imin]) / (float)pthisMagBuffer->iMagBufferCount) /
            fabsf(ftmp);
        ...
        // compute the normalized ellipsoid matrix A with unit determinant and derive invW also with
        // unit determinant.
        ...
        pthisMagCal->ftrinvW[CHX][CHY] = pthisMagCal->ftrinvW[CHX][CHZ] =
            pthisMagCal->ftrinvW[CHY][CHZ] = 0.0F;

        // each element of fA has been scaled by fdetA^(-1/3) so the square of geomagnetic field
        // strength B^2 must be adjusted by the same amount and B by the square root to keep the
        // ellipsoid equation valid: (Bk-V)^T.A.(Bk-V) = (Bk-V)^T.(invW)^T.invW.(Bk-V) = B^2
        pthisMagCal->ftrB *= sqrt(fabs(ftmp));

        // compute the final hard iron offset (uT) by adding in previously subtracted zero mean
        // offset (counts)
        for (i = CHX; i <= CHZ; i++)
            pthisMagCal->ftrV[i] =
                (pthisMagCal->ftrV[i] + (float)pthisMagCal->iMeanBs[i]) * pthisMag->fuTPerCount;
...
void fUpdateMagCalibration10Slice(
    struct MagCalibration *pthisMagCal,
    struct MagBuffer *pthisMagBuffer,
    struct MagSensor *pthisMag)
{
...
#define MATRIX_10_SIZE 10
    ...
    // time slice 0: 18.7k KL25Z ticks for 300 measurements = 0.39ms on KL25Z (variable) stored in
    // systick[0] zero measurement matrix fmatA and calculate the mean values in the magnetic buffer
    if (pthisMagCal->itimeslice == 0)
    {
        int16 iM;  // number of measurements in the magnetic buffer
        ...
        for (i = 0; i < MATRIX_10_SIZE; i++)
            for (j = i; j < MATRIX_10_SIZE; j++) pthisMagCal->fmatA[i][j] = 0.0F;
    ...
    // time slices 1 to MAGBUFFSIZEX * MAGBUFFSIZEY: accumulate matrices: 17.9k KL25Z ticks = 0.37ms
    // for 300 measurements (with max measured stored in systick[1])
    else if (
        (pthisMagCal->itimeslice >= 1) && (pthisMagCal->itimeslice <= MAGBUFFSIZEX * MAGBUFFSIZEY))
    {
        // accumulate the symmetric matrix fmatA on the zero mean measurements
        ...
        if (pthisMagBuffer->index[i][j] != -1)
        {
            ...
            // update non-zero elements fmatA[0-5][9] of fmatA ignoring fmatA[9][9] which is set
            // later. elements fmatA[6-8][9] are zero as a result of subtracting the mean value
            for (k = 0; k < 6; k++) pthisMagCal->fmatA[k][9] += pthisMagCal->fvecA[k];
    ...
    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 1: 1.2k ticks on KL25Z = 0.025ms on KL25Z (constant)
    // (stored in systick[2]) re-enable magnetic buffer for writing and prepare fmatA, fmatB, fvecA
    // for eigendecomposition
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 1))
    {
        // set fmatA[9][9] to the number of magnetic measurements found
        pthisMagCal->fmatA[MATRIX_10_SIZE - 1][MATRIX_10_SIZE - 1] =
            (float)pthisMagBuffer->iMagBufferCount;
        ...
        for (i = 0; i < MATRIX_10_SIZE; i++)
        {
            for (j = 0; j < MATRIX_10_SIZE; j++) pthisMagCal->fmatB[i][j] = 0.0F;
            pthisMagCal->fmatB[i][i] = 1.0F;
            pthisMagCal->fvecA[i] = pthisMagCal->fmatA[i][i];
        }
    ...
    // repeating 45 time slices MAGBUFFSIZEX * MAGBUFFSIZEY + 2 to MAGBUFFSIZEX * MAGBUFFSIZEY + 46
    // inclusive to perform the eigendecomposition of the measurement matrix fmatA. 28.2k ticks =
    // 0.56ms on KL25Z (with max stored in systick[3]). for a 10x10 matrix there are 45 above
    // diagonal elements: 9+8+7+6+5+4+3+2+1+0=45
    else if (
        (pthisMagCal->itimeslice >= (MAGBUFFSIZEX * MAGBUFFSIZEY + 2)) &&
        (pthisMagCal->itimeslice <= (MAGBUFFSIZEX * MAGBUFFSIZEY + 46)))
    {
        // set k to the matrix element of interest in range 0 to 44 to be zeroed and set row i and
        // column j
        k = pthisMagCal->itimeslice - (MAGBUFFSIZEX * MAGBUFFSIZEY + 2);
        if (k < 9)
        {
        ...
        // increment the time slice for the next iteration
        (pthisMagCal->itimeslice)++;
    }  // end of time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 2 to MAGBUFFSIZEX * MAGBUFFSIZEY + 46
       // inclusive

    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 47: 5.6k ticks on KL25Z = 0.12ms on KL25Z (constant)
    // (stored in systick[4]) compute the sum of the absolute values of above diagonal elements in
    // fmatA as eigen-decomposition exit criterion
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 47))
    {
        ...
        fresidue = 0.0F;
        for (i = 0; i < MATRIX_10_SIZE; i++)
            for (j = i + 1; j < MATRIX_10_SIZE; j++) fresidue += fabsf(pthisMagCal->fmatA[i][j]);
    ...
    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 48: 38.5k ticks = 0.80ms on KL25Z (constant) (stored
    // in systick[5]) compute the calibration coefficients (excluding invW) from the solution
    // eigenvector
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 48))
    {
        ...
        for (i = CHX; i <= CHZ; i++)
        {
            pthisMagCal->ftrV[i] = pthisMagCal->finvA[i][0] * pthisMagCal->fmatB[6][imin] +
                                   pthisMagCal->finvA[i][1] * pthisMagCal->fmatB[7][imin] +
                                   pthisMagCal->finvA[i][2] * pthisMagCal->fmatB[8][imin];
            pthisMagCal->ftrV[i] *= -0.5F;
        }

        // compute the geomagnetic field strength B (counts) for current (un-normalized) ellipsoid
        // matrix determinant.
        ftmp = pthisMagCal->fA[CHX][CHY] * pthisMagCal->ftrV[CHX] * pthisMagCal->ftrV[CHY] +
               pthisMagCal->fA[CHX][CHZ] * pthisMagCal->ftrV[CHX] * pthisMagCal->ftrV[CHZ] +
               pthisMagCal->fA[CHY][CHZ] * pthisMagCal->ftrV[CHY] * pthisMagCal->ftrV[CHZ];
        ftmp *= 2.0F;
        ftmp -= pthisMagCal->fmatB[9][imin];
        ftmp += pthisMagCal->fA[CHX][CHX] * pthisMagCal->ftrV[CHX] * pthisMagCal->ftrV[CHX];
        ftmp += pthisMagCal->fA[CHY][CHY] * pthisMagCal->ftrV[CHY] * pthisMagCal->ftrV[CHY];
        ftmp += pthisMagCal->fA[CHZ][CHZ] * pthisMagCal->ftrV[CHZ] * pthisMagCal->ftrV[CHZ];
        ...
        // convert the trial geomagnetic field strength B from counts to uT for un-normalized soft
        // iron matrix A
        ...
        // prepare for eigendecomposition of fA to compute finvW from the square root of fA by
        // setting fmatA (upper left) to the 3x3 matrix fA, set fmatB (eigenvectors to identity
        // matrix and fvecA (eigenvalues) to diagonal elements of fmatA = fA
        for (i = 0; i < 3; i++)
        {
            for (j = 0; j < 3; j++)
            {
    ...
    // repeating 3 time slices MAGBUFFSIZEX * MAGBUFFSIZEY + 49 to MAGBUFFSIZEX * MAGBUFFSIZEY + 51
    // inclusive 9.7kticks = 0.2ms on KL25Z (with max stored in systick[6]). perform the
    // eigendecomposition of the 3x3 ellipsoid matrix fA which has 3 above diagonal elements:
    // 2+1+0=3
    else if (
        (pthisMagCal->itimeslice >= (MAGBUFFSIZEX * MAGBUFFSIZEY + 49)) &&
        (pthisMagCal->itimeslice <= (MAGBUFFSIZEX * MAGBUFFSIZEY + 51)))
    {
        // set k to the matrix element of interest in range 0 to 2 to be zeroed and set row i and
        // column j
        ...
        if (fabsf(pthisMagCal->fmatA[i][j]) > 0.0F)
            fComputeEigSlice(pthisMagCal->fmatA, pthisMagCal->fmatB, pthisMagCal->fvecA, i, j, 3);

        // increment the time slice for the next iteration
        (pthisMagCal->itimeslice)++;
    }  // end of time slices MAGBUFFSIZEX * MAGBUFFSIZEY + 49 to MAGBUFFSIZEX * MAGBUFFSIZEY + 51
       // inclusive

    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 52: 0.3k ticks = 0.006ms on KL25Z (constant) (stored
    // in systick[7]) determine whether the eigen-decomposition of fA is complete
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 52))
    {
        ...
        // to re-enter the eigen-decomposition or skip to calculation of the calibration
        // coefficients
        fresidue = fabsf(pthisMagCal->fmatA[0][1]) + fabsf(pthisMagCal->fmatA[0][2]) +
                   fabsf(pthisMagCal->fmatA[1][2]);
        if (fresidue > 0.0F)
            // continue the eigen-decomposition
            (pthisMagCal->itimeslice) = MAGBUFFSIZEX * MAGBUFFSIZEY + 49;
        else
    ...
    // time slice MAGBUFFSIZEX * MAGBUFFSIZEY + 53: 9.3k ticks = 0.19ms on KL25Z (constant) (stored
    // in systick[8]) compute the inverse gain matrix invW from the eigendecomposition of the
    // ellipsoid matrix A
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 53))
    {
        // set pthisMagCal->fmatB to be eigenvectors . diag(sqrt(sqrt(eigenvalues))) = fmatB .
        // diag(sqrt(sqrt(fvecA))
        ...
        for (i = 0; i < 3; i++)
        {
            for (j = i; j < 3; j++)
            {
                pthisMagCal->ftrinvW[i][j] = pthisMagCal->ftrinvW[j][i] =
                    pthisMagCal->fmatB[i][0] * pthisMagCal->fmatB[j][0] +
                    pthisMagCal->fmatB[i][1] * pthisMagCal->fmatB[j][1] +
                    pthisMagCal->fmatB[i][2] * pthisMagCal->fmatB[j][2];
            }
        }
    ...
    struct MagSensor *pthisMag)
{
    ...
    for (j = 0; j < MAGBUFFSIZEX; j++)
    {
        for (k = 0; k < MAGBUFFSIZEY; k++)
        {
            if (pthisMagBuffer->index[j][k] != -1)
            {
                ...
                {
                    pthisMagCal->fvecA[l] =
                        (float)((int32)pthisMagBuffer->iBs[l][j][k] - (int32)iOffset[l]) * fscaling;
    ...
    for (i = 0; i < 4; i++)
    {
        for (j = i; j < 4; j++)
        {
            pthisMagCal->fmatB[i][j] = pthisMagCal->fmatB[j][i] = pthisMagCal->fmatA[j][i] =
                pthisMagCal->fmatA[i][j];
        }
    }
    ...
    pthisMagCal->ftrB = sqrtf(
        pthisMagCal->fvecA[3] + pthisMagCal->ftrV[CHX] * pthisMagCal->ftrV[CHX] +
        pthisMagCal->ftrV[CHY] * pthisMagCal->ftrV[CHY] +
    ...
    {
        pthisMagCal->ftrV[l] =
            pthisMagCal->ftrV[l] * DEFAULTB + (float)iOffset[l] * pthisMag->fuTPerCount;
    }
    ...
    struct MagSensor *pthisMag)
{
    ...
    for (j = 0; j < MAGBUFFSIZEX; j++)
    {
        for (k = 0; k < MAGBUFFSIZEY; k++)
        {
            if (pthisMagBuffer->index[j][k] != -1)
            {
                ...
                // accumulate the on-and above-diagonal terms of pthisMagCal->fmatA=Sigma{fvecA^T *
                // fvecA} with the exception of fmatA[6][6] which will sum to the number of
                // measurements and remembering that fvecA[6] equals 1.0F update the right hand
                // column [6] of fmatA except for fmatA[6][6]
                for (m = 0; m < 6; m++)
                {
    ...
    fEigenCompute10(pthisMagCal->fmatA, pthisMagCal->fvecA, pthisMagCal->fmatB, 7);
    ...
    // set ellipsoid matrix A to the solution vector with smallest eigenvalue, compute its
    // determinant and the hard iron offset (scaled and offset)
    ...
    // set ftmp to the square of the trial geomagnetic field strength B (counts times
    // FMATRIXSCALING)
    ...
    // convert the geomagnetic field strength B into uT for normalized soft iron matrix A and
    // normalize
    pthisMagCal->ftrB = sqrtf(fabsf(ftmp)) * DEFAULTB * powf(det, -(ONESIXTH));

    // compute trial invW from the square root of A also with normalized determinant and hard iron
    // offset in uT
    f3x3matrixAeqI(pthisMagCal->ftrinvW);
    for (l = CHX; l <= CHZ; l++)
    {
    ...
    struct MagSensor *pthisMag)
{
    ...
    for (j = 0; j < MAGBUFFSIZEX; j++)
    {
        for (k = 0; k < MAGBUFFSIZEY; k++)
        {
            if (pthisMagBuffer->index[j][k] != -1)
            {
                // use first valid magnetic buffer entry as estimate for offset to help solution
                // (bit counts)
                if (iCount == 0)
                {
                    for (l = CHX; l <= CHZ; l++)
    ...
    // set pthisMagCal->fvecA to the unsorted eigenvalues and fmatB to the unsorted normalized
    // eigenvectors of fmatA
    fEigenCompute10(pthisMagCal->fmatA, pthisMagCal->fvecA, pthisMagCal->fmatB, 10);
    ...
    pthisMagCal->ftrB = sqrtf(fabsf(
        pthisMagCal->fA[0][0] * pthisMagCal->ftrV[CHX] * pthisMagCal->ftrV[CHX] +
        ...
        pthisMagCal->fA[2][2] * pthisMagCal->ftrV[CHZ] * pthisMagCal->ftrV[CHZ] -
        pthisMagCal->fmatB[9][j]));

    // calculate the trial normalized fit error as a percentage
    pthisMagCal->ftrFitErrorpc =
        50.0F * sqrtf(fabsf(pthisMagCal->fvecA[j]) / (float)pthisMagBuffer->iMagBufferCount) /
        (pthisMagCal->ftrB * pthisMagCal->ftrB);

    // correct for the measurement matrix offset and scaling and get the computed hard iron offset
    // in uT
    for (l = CHX; l <= CHZ; l++)
    {
        pthisMagCal->ftrV[l] =
    ...
    fEigenCompute10(pthisMagCal->fmatA, pthisMagCal->fvecA, pthisMagCal->fmatB, 3);
